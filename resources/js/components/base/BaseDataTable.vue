      ...
      <table class="min-w-full">
        <thead>
          <tr v-for="headerGroup in table.getHeaderGroups()" :key="headerGroup.id"
            ...
            <th v-for="header in headerGroup.headers" :key="header.id"
                :class="[
                  'px-5 py-3 text-left font-medium text-gray-500 text-theme-xs dark:text-gray-400',
                  header.column.getCanSort() ? 'cursor-pointer select-none' : '',
                ]"
                @click="header.column.getToggleSortingHandler()?.($event)">
              <div class="flex items-center gap-1">
                <FlexRender :render="header.column.columnDef.header" :props="header.getContext()" />
                <span v-if="header.column.getIsSorted()" class="text-gray-400">{{ header.column.getIsSorted() === 'asc' ? '▲' : '▼' }}</span>
              </div>
            </th>
          </tr>
        </thead>
        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
          ...
          <tr v-if="table.getRowModel().rows.length === 0">
            <td :colspan="table.getAllColumns().length" class="px-5 py-8 text-center text-gray-400 text-theme-sm dark:text-gray-500">
              No results found.
            </td>
          </tr>
        </tbody>
...
const pageSizeModel = computed({
  ...
  set: (val: number) => { pagination.value = { ...pagination.value, pageSize: val, pageIndex: 0 } },
})
...
const table = useVueTable({
  ...
  state: {
    ...
    get globalFilter() { return filterModel.value },
  ...
  onGlobalFilterChange: (updaterOrValue) => {
    const newValue = typeof updaterOrValue === 'function' ? updaterOrValue(filterModel.value) : updaterOrValue
...
const visiblePages = computed(() => {
  ...
  if (currentPage <= 1) { end = Math.min(3, pageCount - 2) }
  if (currentPage >= pageCount - 3) { start = Math.max(1, pageCount - 5) }
