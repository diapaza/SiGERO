...
- **Límites de longitud en inputs**: los campos con tope (DNI 8, WhatsApp 9, nombres/apellidos 120, código de objeto 4 o 12, etc.) restringen el tipeo con el atributo nativo `maxlength` (soportado por `BaseInput`, `BasePasswordInput` y `BaseTextarea`), además de las reglas espejo de `useValidation` (frontend) y FormRequests (backend). `whatsapp_number` es obligatorio y debe tener exactamente 9 dígitos (columna `char(9)` NOT NULL; la migración rellena `000000000` las filas previas con NULL).
